...
def histgram(cube_dir,name,out_dir_name):
    ...
    for smiles in df["smiles"]:  # [:100]:
        print(smiles)
        mol = calculate_conformation_review.get_mol(smiles)
...
if __name__ == '__main__':
    ...
    #     print(len(df))
    #     dfs.append(df)
    df = pd.concat([pd.read_excel(path) for path in glob.glob("../../../arranged_dataset/review/*.xlsx")]).dropna(subset=['smiles']).drop_duplicates(subset=["smiles"])
    df["mol"] = df["smiles"].apply(calculate_conformation_review.get_mol)
    #cubeのディレクトリを指定。
    dir="D:\calculation\wB97X-D_def2-TZVP20240416_review"#"C:/Users/poclabws/calculation/wB97X-D_def2-TZVP20240416"#"F:/wB97X-D_def2-TZVP20240416"
    print(df.columns)
    
    
    df = df[[os.path.isdir(dir + "/" + mol.GetProp("InChIKey")) for mol in df["mol"]]]
    df["mol"].apply(
        lambda mol: calculate_conformation_review.read_xyz(mol, dir + "/" + mol.GetProp("InChIKey")))
    ...
    out_dir_name="../../../grid_coordinates/20240606_review_"+str(interval).replace('.', '_')
    ...
    for mol in df["mol"]:
        ...
        for conf in mol.GetConformers():
            confid =conf.GetId()
            input = dir + "/" + mol.GetProp("InChIKey")+"/data{}.pkl".format(confid), dfp, out_dir_name + "/" + mol.GetProp("InChIKey")+"/data{}.pkl".format(confid)
            inputs.append(input)
