...
for file, label in zip(glob.glob("../arranged_dataset/*.xlsx")[1:],
    ...
    for _ in range(5):
        ...
        # Gaussian2.append(dfp["Gaussian_test_r22"].values.tolist())
        # Gaussian3.append(dfp["Gaussian_test_r23"].values.tolist())
        # Gaussian4.append(dfp["Gaussian_test_r24"].values.tolist())
        # Gaussian5.append(dfp["Gaussian_test_r25"].values.tolist())
    ...
    #         label="Gaussian3\nMAX{:.2f}".format(np.max(np.average(Gaussian3, axis=0))), color=cm.hsv(3/5), alpha=1)
    # ax.plot(dfp["lambda"], np.average(Gaussian4, axis=0), "o",
    #          label="Gaussian4\nMAX{:.2f}".format(np.max(np.average(Gaussian4, axis=0))), color=cm.hsv(4/5), alpha=1)
    # ax.plot(dfp["lambda"], np.average(Gaussian5, axis=0), "o",
    #          label="Gaussian5\nMAX{:.2f}".format(np.max(np.average(Gaussian4, axis=0))), color=cm.hsv(5/5), alpha=1)
    # ax.plot([], [], "o",
...
for file, label in zip(glob.glob("../arranged_dataset/*.xlsx"),
    ...
    for _ in range(100):
        ...
        ax.plot(dfp["ΔΔG.expt."], dfp["Gaussian_predict"], "o", label="Gaussian" if _ == 1 else None, color='blue',
                markeredgecolor="none", alpha=0.1)
        ax.plot(dfp["ΔΔG.expt."], dfp["Ridge_predict"], "o", label="Ridge" if _ == 1 else None, color="red",
                markeredgecolor="none", alpha=0.1)
        ax.plot(dfp["ΔΔG.expt."], dfp["Lasso_predict"], "o", label="Lasso" if _ == 1 else None, color="green",
                markeredgecolor="none", alpha=0.1)
        ax.plot(dfp["ΔΔG.expt."], dfp["PLS_predict"], "o", label="PLS" if _ == 1 else None, color="orange",
                markeredgecolor="none", alpha=0.1)
