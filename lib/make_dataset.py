...
def output(df,to_file_path):
    ...
    df["aldehyde"]=df["ROMol"].map(lambda mol: mol.HasSubstructMatch(Chem.MolFromSmarts("[2H]C(=O)[#6]")))
    ...
    print(len(df[df["aliphatic_aliphatic"]&~df["ring"]&~df["test"]]),len(df[df["aliphatic_aliphatic"]&~df["ring"]&df["test"]]),
          ...
          len(df[df["aldehyde"]&~df["test"]]),len(df[df["aldehyde"]&df["test"]]),
          len(df),len(df[df["test"]==0]),len(df[df["test"]==1]))

if __name__ == '__main__':
    df_cbs=common("/Users/mac_poclab/PycharmProjects/CoMFA_model/sampledata_local/CBS.xlsx")
    df_dip=common("/Users/mac_poclab/PycharmProjects/CoMFA_model/sampledata_local/DIP.xlsx")
    df_ru=common("/Users/mac_poclab/PycharmProjects/CoMFA_model/sampledata_local/alpine_borane.xlsx")
    ...
    df_dip=df = df_dip[df_dip["mol"].map(lambda mol:
                              not  mol.HasSubstructMatch(Chem.MolFromSmarts("[Li]"))
